...
export default function ElyPage() {
  ...
  const dlftlrycpList = [
    { id: "a4", name: "영웅왕의 망토", supply: "경매장 구매 또는 직작", desc: "아무 옵션 10레벨 달려있으면 프리미엄샵에서 엘획 64% 이상(최대 66%) 뜰 때까지 옵션 재분배 돌리기 - 경매장 구매 또는 별의 요람 파밍 ", color: "warning", percent: 64 },
  ...
  const tkdtlList = [
    { id: "c1", name: "참", supply: "경매장 구매 또는 파밍", desc: "수르트의 참(60) 없으면 임시로 카이로스의 참(50)/미카엘라의 참(50)/학원장의 참(49)/파라오의 참(40)/나비모양의그림자 참(30) 장착", color: "warning", percent: 60 },
    { id: "c2", name: "아가슈라의 4번젬 x3", supply: "경매장 구매", desc: "여유있을 때 92% 이상(최대 106%) 구매 - 엘획만 있는거 구매했다면 수시로 교체하면되고 스펙+엘던용이면 항상끼면됨", color: "danger", percent: 276 }, //최대 318
  ]
  ...
  const rPwjdList = [
    ...
    { id: "f5", name: "별 각인 (30일)", supply: "하급엘던에서 수급 또는 경매장 구매", desc: "75% 이상(최대 80%) 나올 때까지 돌리기", color: "warning", percent: 75 },
  ...
  const zoflrList = [
    ...
    { id: "e4", name: "직업", supply: "윈드스토커 패시브 스킬", desc: "백만장자(60) / 대부호(70) / 골드러쉬(80)", color: "warning", percent: 210 },
  ]
  ...
  const checkboxString = (item: any) => {
    return (
      <Card className={`w-[360px] sm:w-[560px] border-2 border-${item.color}`}>
        <CardHeader className="justify-between gap-3">
          <div className="flex gap-5">
            <div className="flex flex-col gap-1 items-start justify-center">
              ...
              <h5 className="w-full text-small text-left tracking-tight text-default-400">{item.supply}</h5>
            </div>
          </div>
  ...
  return (
    <div>
      ...
      <div className="flex gap-1 justify-center py-2">
        <Chip size="sm" color="warning" variant="bordered">쉬운거 914%</Chip>
        <Chip size="sm" color="primary" variant="bordered">꾸준히 955%</Chip>
        <Chip size="sm" color="danger" variant="bordered">비싼거 1631%</Chip>
      </div>
